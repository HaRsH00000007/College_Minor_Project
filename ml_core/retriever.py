...
class ProductRetriever:
    ...
    def __init__(self, embeddings: Optional[np.ndarray], df: pd.DataFrame, index_type: str = "IndexFlatIP"):
        self.df = df.reset_index(drop=True)
        self.index_type = index_type
        self.index = None

        if embeddings is None:
            logger.warning("Embeddings are None — FAISS index not built yet.")
